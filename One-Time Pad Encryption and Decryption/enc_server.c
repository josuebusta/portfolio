...
void setupAddressStruct(struct sockaddr_in* address, 
  ...
  if (hostInfo == NULL) { 
    fprintf(stderr, "ENC_SERVER: ERROR, no such host\n"); 
    exit(1); 
  }
...
void encrypt(char *plain, char *keygen, char *cipher){
    int plainInd = 0;
    int keyInd = 0;
    int newInd = 0;
...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    encrypt(plain, keygen, cipher);
    sendText(connectionSocket, (char *)&size, sizeof(int));
    sendText(connectionSocket, cipher, size + 1);
