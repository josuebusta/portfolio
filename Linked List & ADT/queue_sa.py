...
# Assignment: 3
# Due Date: 5/6/2024
# Description: A Queue ADT class implementation containing methods to enqueue,
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        """
        Takes a value as a parameter and adds that value to the end of the queue.
        """

        # CHECKS if array is filled and needs resizing
        if self.size() == self._sa.length():
            self._double_queue(self._sa.length())

        # ADDS value to the end of the queue
        self._sa.set(self._increment(self._back), value)
        self._back = self._increment(self._back)
        self._current_size += 1

    def dequeue(self) -> object:
        """
        Removes a value from the start of the queue and returns that value.
        """
        ...
        # REMOVES value from start of queue
    ...
    def _double_queue(self, old_capacity: int) -> None:
        """
        Takes a static array's capacity as a parameter and adds the values
        from the original array to a new array with double the capacity.
        """

        # INITIATES new array with double the capacity
        new_capacity = old_capacity * 2
        old_arr = self._sa
        new_arr = StaticArray(new_capacity)

        # ADDS elements from original array to larger array
        new_index = self._front
        for index in range(old_capacity):
            new_arr.set(index, old_arr.get(new_index))
            new_index = self._increment(new_index)
